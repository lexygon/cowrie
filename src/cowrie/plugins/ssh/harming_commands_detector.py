...
class HarmingCommandsDetectorPlugin(BasePlugin):
    command_list = [
        {
            "command": "rm"
        },
    ...
    def process_event(self, event):
        ...
        if found_list:
            self.logger.msg(eventid='cowrie.command.harming_command', input=_input, found_list=found_list,
                            format='Found harming commands in command "%(input)s": %(found_list)s')
